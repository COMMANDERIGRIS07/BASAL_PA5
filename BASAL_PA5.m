...
%% Submitted by: Earl David M. Basal
%% 2ECE-C
...
% For real matrices: A', A.', and transpose(A) all behave the same.
% For complex matrices:
transpose(A) %    * transpose(A) → transpose only (same as A.')
...
transpose(B) %    * transpose(B) → transpose only (same as B.')
B'           %    * B'  → transpose + conjugate
B.'          %    * B.' → transpose only
